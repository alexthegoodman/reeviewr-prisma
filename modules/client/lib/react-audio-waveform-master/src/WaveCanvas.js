...
class WaveCanvas extends React.Component {
  ...
  drawBars = (waveCanvasCtx, width, peaks) => {
    ...
    //   console.info("peaks", key, peaks[key]);
    // }
    for (let i = 0; i < width; i += step) {
      let h = Math.round((peaks[Math.floor(i * scale)] / absmax) * halfH);
      let h2 = Math.round((peaks[Math.floor(i * 1)] / absmax) * halfH);
      let h3 = Math.round((peaks[Math.floor(i * scale)] / 1) * halfH);
      let h4 = Math.round((peaks[Math.floor(i * scale)] / absmax) * 1);
